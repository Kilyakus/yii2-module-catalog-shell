...
class Category extends \bin\admin\models\CategoryModel
{
	...
	public function getFieldList()
	{
		...
		if($this->parent)
		{
			...
			{
				$fields = CField::find()->where(['or',['category_id' => $parent->category_id],['is','category_id', new Expression('null')]])->andFilterWhere(['class' => $Category])->andFilterWhere(['status' => 1])->all();
		...
		$fields = CField::find()->where(['or',['category_id' => $this->category_id],['is','category_id', new Expression('null')]])->andFilterWhere(['class' => $Category])->andFilterWhere(['status' => 1])->all();
		foreach ($fields as $field) {
			$this->_fields[$field->field_id] = $field;
		}
	...
	public function getContacts()
	{
		...
		if($this->parent){
			...
			foreach ($parents as $parent) {
				$fields = CContact::find()->where(['or',['category_id' => $parent->category_id],['is','category_id', new Expression('null')]])->andFilterWhere(['class' => $Category])->andFilterWhere(['status' => 1])->all();
				foreach ($fields as $field) {
		...
		$fields = CContact::find()->where(['or',['category_id' => $this->category_id],['is','category_id', new Expression('null')]])->andFilterWhere(['class' => $Category])->andFilterWhere(['status' => 1])->all();
		foreach ($fields as $field) {
			$this->_contacts[$field->contact_id] = $field;
		}
