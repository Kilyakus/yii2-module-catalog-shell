...
use kilyakus\modules\behaviors\SortableDateController;
        ...
        if($image != false){
            $image = Html::img(
                Image::thumb($field->image, 34,34),[
                    'class' => 'img-rounded img-responsive border border-darkgray',
            ...
            return $label.'<div class="row v-align"><div class="pl-15" style="min-width:50px;">'.$image.'</div><div class="col-xs-12">'.$html.'</div></div>';
        }else{
            return $label.$html;
        }
    }
    ...
    public function getItems($children = null,$data = null,$field = null)
    {
        ...
        elseif ($field->type === 'select' && self::table($field) != true) {
            if($field->options){
                ...
                $html = Select2::widget(['id' => $field->name . '-' . $field->field_id, 'name' => 'Data['.$field->name.']','theme' => 'default', 'data' => $options, 'value' => $value]);
                $result .= self::genContainer($html,$field,true);
        ...
        elseif ($field->type === 'checkbox' && self::table($field) != true) {
            ...
            if($field->options){
                ...
                $html = Select2::widget(['id' => $field->name . '-' . $field->field_id, 'name' => 'Data['.$field->name.']','theme' => 'default', 'data' => $options, 'value' => $value, 'options' => ['multiple' => true], 'pluginOptions' => ['closeOnSelect' => false]]);
                $result .= self::genContainer($html,$field,true);
